...
4. Claude: provides complete answers 

Requirements:
- Maximum 2 sequential rounds per user query
- Terminate when: (a) 2 rounds complete, (b) Claude response has no tool_use blocks, or (c) tool calls fails
- reserve conversation context between rounds
- Handle tools execution errors gracefully

Notes:
- uodate the system prompt in @backend/ai_generator.py
- update the test @backend/tests/test_ai_generator.py
- Write test that verify the external behavior (API calls made, tools executed, return results) rather than internal state details
